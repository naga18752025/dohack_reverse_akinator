  ...
  try {
    ...
    if(prompt === 0){
      ...
      if(typeOfPrompt === 1){
        ...
        realPrompt = `
        あなたの役割：以下の条件に厳密に従い、"名詞を1語だけ" 出力してください。
        目的：逆アキネーターゲームのお題（日本人のほとんど全員に知られているが、お題としては思い浮かびにくい名詞）を**無作為に1つ選ぶ**こと。
        ...
        - 思い浮かべにくい語を選ぶにあたって、**専門的・知識が必要な語を選ぶことは許されない**。

        5) recent avoidance（直近採用回避）
        - 変数 ${themeLog} に含まれる語（直近30件）は**避ける**。
        - なるべく直近ログ ${themeLog} と**カテゴリが異なる**ものを選ぶよう努める。

        以上の条件に従って、**無作為に1語の名詞（お題として思い浮かべにくいやつ）**だけを出力してください。`;
      }
    }else if(prompt === 0.5){
