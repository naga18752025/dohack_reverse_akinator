  ...
  try {
    ...
    if(prompt === 0){
      ...
      if(typeOfPrompt === 1){
        ...
        - 国民的キャラクターはOK（例：「ドラえもん」「ピカチュウ」「クレヨンしんちゃん」）
        - 有名な作品やシリーズはOK（例：「アベンジャーズ」「ポケモン」）
        - 有名な地名・国名はOK（例：「東京」「ロンドン」「アメリカ」）
        - 一般的に誰でも知っているブランド・製品はOK（例：「コーラ」「ポケモン」）
        - **形容詞などを名詞化した語は避けてください**（例：「やさしさ」「かたさ」）
        - 漢字・ひらがな・カタカナの一般的表にしてください
        ...
        realPrompt = `
        あなたの役割：以下の条件に厳密に従い、"名詞を1語だけ" 出力してください。
        目的：逆アキネーターゲームのお題（日本人に広く知られているが、思い浮かびにくい名詞）を**無作為に1つ選ぶ**こと。
