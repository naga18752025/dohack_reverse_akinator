...
app.post("/api/openai", async (req, res) => {
  try {
    ...
    if(prompt === 0){
      ...
      const typeOfPrompt = Math.random() < 0.5 ? 0 : 1;
      ...
      if(typeOfPrompt === 1){
        ...
        - なるべく直近ログと**カテゴリが異なる**ものを選ぶよう努めよ（完全に別カテゴリにできない場合は語自体を避ける）。
      ...
      ただし、ひらがな・カタカナ・漢字以外の記号などが文字列に混じっていたり日本語の名詞として存在しない不適切な文字列であった場合は絶対に「わかりません」とだけ出力してください。
      ...
      -ひらがな・カタカナ・漢字以外ののものが文字列に混じっていたり日本語の名詞として存在しない不適切な文字列であった場合は絶対に「わかりません」とだけ出力してください。
      `;
    }else if(prompt === 1){
      const selectedCharacter = info2;
      const question = Q;
